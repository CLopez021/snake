...
(* Exercise 03a:
   ...
   This module is a simple record that represent the parameters of the board.
   It is a record with two fields, [height] and [width].
...
let in_bounds t { Position.row; col } =
  not (row < 0 || row >= t.height || col < 0 || col >= t.width)
;;
