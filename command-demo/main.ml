...
(* In OCaml, we use the [Command] library to write apps that take commandline
   arguments.

   You should understand generally how to construct apps that use [Command],
   although you are allowed to treat this roughly as a magic incantation
   template that you can copy-paste every time you need it in your own app.

   Here, we'll demonstrate a basic usage of [Command] and the type of
   commandline arguments we can create using it.

   You can run this program with {[ $ cd snake/command-demo $ dune exec
   ./main.exe -- -help ]}

   The "dune exec" invocation should be familiar as the way that you ran the
   snake game. Afterwards, we need the double hyphen "--" to pass any other
   arguments into the main.exe binary. (You can accept this as an incantation
   for passing arguments when running OCaml binaries using "dune exec", but
   ask a TA if you want to know more.)

   The command above will show you some help related to the arguments you
   need to pass to run the program. Try this:

   {[ $ dune exec ./main.exe -- -name "Alice" ]}

   Now it should actually run, and print something to stdout.

   Take a look at the following code, and try to use the other two supported
   commandline arguments as well. *)
