...
(*let%expect_test "score changes when apple is consumed" = Random.init 42;
  ...
  Apple.Exercises.create_with_location Position.{ row = 0; col = 4 } in
  ...
  works properly. If you believe this is the case, please check in with a TA,
  and comment out this test to continue.|}; [%expect {| Score changed after
  consuming apple |}] ;; *)
